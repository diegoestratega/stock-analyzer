...
# --- 4. VERCEL NATIVE HANDLER USING GET ---
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Parse the URL to get the ticker parameter
        parsed_url = urlparse(self.path)
        query_params = parse_qs(parsed_url.query)
        
        ticker_list = query_params.get('ticker')
        if not ticker_list:
            self.wfile.write(json.dumps({"detail": "No ticker provided in URL parameters."}).encode('utf-8'))
            return
